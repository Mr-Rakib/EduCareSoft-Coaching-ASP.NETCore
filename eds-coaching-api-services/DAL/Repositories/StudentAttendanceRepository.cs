...
{
    public class StudentAttendanceRepository : ICRUDRepository<Attendance>
    {
        ...
        public List<Attendance> FindAll()
        {
            ...
            try
            {
                ...
                {
                    using (command = new MySqlCommand(Views.ALLStudentAttendance, connection))
                    {
                        connection.Open();
                        ...
                        using (reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                Attendance Attendance = new Attendance
                                {
                                    ...
                                    User_id = reader.GetInt32("student_id"),
        ...
        public bool Update(Attendance Attendance)
        {
            ...
            using (connection = DBConnection.GetConnection())
            {
                try
                {
                    using (command = new MySqlCommand(Procedures.UpdateStudentAttendance, connection))
                    {
        ...
        public bool Save(Attendance Attendance)
        {
            ...
            using (connection = DBConnection.GetConnection())
            {
                try
                {
                    using (command = new MySqlCommand(Procedures.SaveStudentAttendance, connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add(new MySqlParameter("@user_id", Attendance.User_id));
                        command.Parameters.Add(new MySqlParameter("@timeIn", Attendance.TimeIn));
        ...
        public bool Delete(int id)
        {
            ...
            using (connection = DBConnection.GetConnection())
            {
                try
                {
                    using (command = new MySqlCommand(Procedures.DeleteStudentAttendance, connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
